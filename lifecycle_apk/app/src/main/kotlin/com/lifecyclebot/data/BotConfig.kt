...
data class BotConfig(
    ...
    // User-supplied public destination for treasury profit splits. If blank,
    // TreasuryWalletManager falls back to its generated/imported treasury keypair.
    val treasuryWalletAddress: String = "",
    ...
    // V5.0.4001 — live quality compounding controls intentionally live
    // outside this giant data-class primary constructor. BotConfig is already
    // near the JVM constructor/default-mask slot limit; adding Double/String
    // params here causes ClassFormatError in tests/runtime. Use the extension
    // defaults below instead until BotConfig is split into nested config groups.
    ...
    val snipeMode: Boolean = false,  // V5.9.982 — z31/z32 SnipeAgeGate enable. Default OFF (preserves current behavior). When ON, tokens >15min get shunted to BACKGROUND lane (NEVER purged — operator override z32).
    ...
    val maxWatchlistSize: Int = 220,             // V5.0.6480: canonical physical cap
...
// V5.0.4002 — constructor-safe live quality defaults. These preserve the 4001
...
object ConfigStore {
    ...
    fun save(ctx: Context, cfg: BotConfig) {
        ...
        prefs(ctx).edit().apply {
            ...
            putString("treasury_wallet_address",      cfg.treasuryWalletAddress)
            ...
            // V5.0.4002 — 4001 live quality defaults are extension-backed
            // to avoid overflowing BotConfig's JVM constructor slot budget.
            // Do not persist them through BotConfig until the config model is split.
    ...
    fun load(ctx: Context): BotConfig {
        ...
        return BotConfig(
            ...
            treasuryWalletAddress       = p.getString("treasury_wallet_address", "") ?: "",
            rpcUrl                      = p.getString("rpc_url", "") ?: "",
            paperMode                   = p.getBoolean("paper_mode", true),
            // V5.9.495z29 — operator spec item 8: throughput controls.
            highThroughputLiveMode      = p.getBoolean("ht_live_mode", true),
            maxLiveTradesPerDay         = p.getInt    ("ht_max_trades_per_day", 1000),
            maxConcurrentLivePositions  = p.getInt    ("ht_max_concurrent_live", 24),
            ...
            // V5.0.4002 — live quality defaults are extension-backed;
            // keep them out of BotConfig's constructor to avoid ClassFormatError.
            ...
            maxWatchlistSize            = p.getInt("max_watchlist_size", 220).coerceIn(100, 220), // V5.0.6480: align with registry source authority
