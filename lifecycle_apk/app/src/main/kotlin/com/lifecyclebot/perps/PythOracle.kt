...
object PythOracle {
    ...
    data class PythPrice(
        ...
        fun ageMs(nowMs: Long = System.currentTimeMillis()): Long = nowMs - publishTime * 1000L

        fun isStale(nowMs: Long = System.currentTimeMillis(), maxAgeMs: Long = 60_000L): Boolean {
            if (publishTime <= 0L) return true
            val age = ageMs(nowMs)
            return age < -5_000L || age > maxAgeMs
        }

        fun getConfidencePct(): Double =
            if (price.isFinite() && price > 0.0 && confidence.isFinite() && confidence >= 0.0) {
                confidence / price * 100.0
            } else {
        ...
        fun isTradable(
            ...
            maxConfidencePct: Double = 1.0,
        ): Boolean =
            price.isFinite() &&
                price > 0.0 &&
                emaPrice.isFinite() &&
                emaPrice > 0.0 &&
                !isStale(nowMs, maxAgeMs) &&
                getConfidencePct() <= maxConfidencePct
    }
    ...
    suspend fun getPrice(symbol: String): PythPrice? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            // V5.0.6637: zero, stale, future-dated, non-finite, or
            // low-confidence data is unavailable—not a tradable fallback.
            if (pythPrice != null) {
                if (loggedDead.add(symbol)) {
                    ErrorLogger.warn(
                        TAG,
                        "Pyth feed rejected for $symbol: price=${pythPrice.price} ageMs=${pythPrice.ageMs()} confPct=${pythPrice.getConfidencePct()}",
                    )
                }
            }
    ...
    // V5.0.6637: Deliberately no synthetic fallback prices. A missing,
    // stale, future-dated, or low-confidence oracle value remains unavailable
    // so downstream execution can fail closed or consult an independent source.
