...
positionMap[position.id] = position
    ...
    private suspend fun executeLiveTradeAtSize(positionId: String, signal: ForexSignal, typeLabel: String, sizeSol: Double): Boolean {
    ...
    private fun closePosition(position: ForexPosition, positionMap: ConcurrentHashMap<String, ForexPosition>, reason: String) {
        ...
        if (!position.isPaper) try {
            TradeHistoryStore.recordTrade(Trade(
                side             = "SELL",
                // V5.0.6665 — terminal mode belongs to the immutable position,
                // not the mutable current-mode toggle.
                mode             = "live",
                sol              = position.size,
    ...
    fun getBalance(): Double = if (isPaperMode.get())
        com.lifecyclebot.engine.truth.GlobalCapitalArbitration6617.availableForLane("FOREX", paperMode = true)
        else com.lifecyclebot.engine.truth.GlobalCapitalArbitration6617.availableForLane("FOREX", paperMode = false, liveWalletSol = liveWalletSol7212())
    ...
    fun resetCounters() {
        ...
        totalPnlSol = 0.0
        saveState()
        ErrorLogger.info(TAG, "🧹 ForexTrader counters reset")
    }
    ...
    private fun liveWalletSol7212(): Double {
        ...
        try {
            com.lifecyclebot.engine.PipelineHealthCollector
                .labelInc("LANE_LIVE_BALANCE_FROM_AUTHORITY_7212_FOREX")
        } catch (_: Throwable) {}
        return authority
    }
