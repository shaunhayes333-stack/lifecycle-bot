    ...
    private suspend fun runScanCycle() {
        ...
        for (market in metalMarkets) {
            try {
                ...
                if (!isPaperMode.get() && (metalsTrust == com.lifecyclebot.v4.meta.TrustLevel.DISTRUSTED)) {
                    ErrorLogger.warn(TAG, "🥇 ${market.symbol}: TRUST_GATE — MetalsAI DISTRUSTED, skipping entry")
                    continue
                }
                if (isPaperMode.get() && metalsTrust == com.lifecyclebot.v4.meta.TrustLevel.DISTRUSTED) {
                    try { com.lifecyclebot.engine.PipelineHealthCollector.labelInc("MARKETS_FUNNEL_6567|FAMILY=METALS|STAGE=PAPER_TRUST_ADVISORY") } catch (_: Throwable) {}
                }
...
positionMap[position.id] = position
    ...
    private fun partialPosition6566(position: MetalPosition, positionMap: ConcurrentHashMap<String, MetalPosition>, reason: String) {
        if (!position.isPaper) {
            try { com.lifecyclebot.engine.PipelineHealthCollector.labelInc("LIVE_PARTIAL_DEFERRED_NO_CONFIRMED_ADAPTER_6566_METAL") } catch (_: Throwable) {}
            return
        }
        ...
        persistMetalPositions()
        try { com.lifecyclebot.engine.PipelineHealthCollector.labelInc("CROSS_ASSET_PARTIAL_APPLIED_6566_METAL") } catch (_: Throwable) {}
    }
    ...
    fun getBalance(): Double = if (isPaperMode.get())
        ...
        else com.lifecyclebot.engine.truth.GlobalCapitalArbitration6617.availableForLane("METALS", paperMode = false, liveWalletSol = liveWalletSol7212())
    ...
    private val preferLeverage = AtomicBoolean(true)   // V5.9.369: leverage default for metals
    ...
    private fun liveWalletSol7212(): Double {
        ...
        if (!authority.isFinite() || authority <= 0.0) return liveWalletBalance
        try {
            com.lifecyclebot.engine.PipelineHealthCollector
                .labelInc("LANE_LIVE_BALANCE_FROM_AUTHORITY_7212_METALS")
        } catch (_: Throwable) {}
        return authority
    }
