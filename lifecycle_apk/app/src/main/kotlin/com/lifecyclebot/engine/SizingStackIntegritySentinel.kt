...
object SizingStackIntegritySentinel {
    ...
    fun inspect(
        ...
        val finding = when {
            rawProduct < 0.18 -> "dust_stack product=${rawProduct.fmt(3)} tiny=${tiny.joinToString("+").ifBlank { "none" }}"
            rawProduct > 4.50 -> "runaway_stack product=${rawProduct.fmt(3)} boost=${boost.joinToString("+").ifBlank { "none" }}"
            tiny.size >= 3 -> "triple_downsize tiny=${tiny.joinToString("+")} product=${rawProduct.fmt(3)}"
            boost.size >= 4 -> "quad_boost boost=${boost.joinToString("+")} product=${rawProduct.fmt(3)}"
            else -> null
        } ?: return null
    ...
    private fun emit(severity: String, reason: String, mode: String, lane: String, source: String, mint: String, symbol: String, rawProduct: Double, components: Map<String, Double>): Finding {
        try {
            val compact = components.entries.sortedBy { it.key }.joinToString(",") { "${it.key}=${it.value.fmt(3)}" }.take(420)
            ForensicLogger.lifecycle("SIZING_STACK_INTEGRITY_SENTINEL_4285", "severity=$severity reason=${reason.take(140)} mode=$mode lane=$lane source=$source mint=${mint.take(10)} symbol=$symbol product=${rawProduct.fmt(3)} components=$compact")
