...
class Executor(
    ...
    // V5.9.780 — short helpers so the 36 mode-decision call sites don't
    // each carry the full FQN string. The bulk replacement of
    // cfg().paperMode → isPaperRT()
    ...
    private fun rehydratePositionFromSubTraders(ts: TokenState): Boolean {
        ...
        // Quality (no isPaper field — infer from isPaperRT())
        try {
    ...
    private fun recordTrade(ts: TokenState, trade: Trade) {
        ...
        val entryMcapForJournal: Double = trade.entryMcapUsd.takeIf { it > 0.0 }
            ...
            // V5.0.3809 — do NOT stamp current/discovery mcap onto SELL/PARTIAL rows.
    ...
    private fun executeProfitLockSell(
        ...
        try {
            ...
            if (sig == null) {
                ...
                val pumpTip = effectiveJitoTipLamports(c, urgent = isDrainExit)
        ...
        if (pos.isPaperPosition) {
            ...
            try {
                ...
                if (pumpSig != null) {
                    ...
                    val useJito = c.jitoEnabled && !quote.isUltra
                    val jitoTip = effectiveJitoTipLamports(c, urgent = false)
                    val ultraReqId = if (quote.isUltra) txResult.requestId else null
                    sig = try {
    ...
    fun maybeAct(
        ...
        if (isPaperRT() && ts.position.isOpen && !ts.position.isFullyBuilt) {
        ...
        if (signal == "BUY") {
            ErrorLogger.debug("Executor", "BUY CHECK: ${ts.symbol} | shouldAct=$shouldActOnBuy | posOpen=${ts.position.isOpen} | autoTrade=${cfg().autoTrade} | paper=${isPaperRT()}")
        }
    ...
    internal fun doBuy(ts: TokenState, sol: Double, score: Double,
        ...
        // (atomic, no cache), not isPaperRT(). See doExecuteBuy site
    ...
    fun paperBuy(ts: TokenState, sol: Double, score: Double, identity: TradeIdentity? = null, 
        ...
        // The existing comment ("only reached when isPaperRT()==true")
    ...
    // paperSell() UNCONDITIONALLY, ignoring isPaperRT() entirely. In
    ...
    private fun liveBuy(ts: TokenState, sol: Double, score: Double,
        ...
        val buyLease = ExecutionAttemptLease.acquire(
            ...
            mint = ts.mint,
            symbol = ts.symbol,
            processor = buyLeaseProcessor,
            mode = if (RuntimeModeAuthority.isPaper()) "PAPER" else "LIVE",
            generation = buyLeaseGeneration,
        )
        if (!buyLease.allowed) {
            try {
                ForensicLogger.exec(
                    "LIVE_BUY_DEFERRED", ts.symbol,
                    "mint=${ts.mint.take(10)} sol=$sol reason=${buyLease.reason} backoffMs=${buyLease.backoffMs} processor=$buyLeaseProcessor",
                )
            } catch (_: Throwable) {}
            return false
        }
        ...
        if (sol <= 0 || sol.isNaN() || sol.isInfinite()) {
            ...
            buyTerminalFail("BUY_TERMINAL_MIN_NOTIONAL_AFTER_FEES:INVALID_SIZE")
            return false
        }
        if (ts.mint.isBlank() || ts.symbol.isBlank()) {
            ...
            buyTerminalFail("BUY_TERMINAL_NO_EXECUTABLE_ROUTE:EMPTY_MINT_OR_SYMBOL")
            return false
        }
        ...
        if (com.lifecyclebot.engine.TokenBlacklist.isBlocked(ts.mint)) {
            ...
            buyTerminalFail("BUY_TERMINAL_NO_EXECUTABLE_ROUTE:BLACKLIST")
            return false
        }
        ...
        // it does not block RugCheck pending, holder-data pending, active/unknown
        ...
        run {
            ...
            if (!preTrade.allowed) {
                ...
                buyTerminalFail("BUY_TERMINAL_NO_EXECUTABLE_ROUTE:PRETRADE_${preTrade.reason}")
                return false
            }
        }
        ...
        when (val decision = LiveExecutionGate.tryAcquireBuy(openLive)) {
            is LiveExecutionGate.Decision.Blocked -> {
                ...
                buyTerminalFail("BUY_TERMINAL_NO_EXECUTABLE_ROUTE:EXEC_GATE_${decision.code}")
                return false
            }
        ...
        if (EmergentGuardrails.shouldBlockMultiLayerEntry(tradeId.mint, currentLayer)) {
            ...
            if (!trulyOpen) {
                ...
                buyTerminalFail("BUY_TERMINAL_DUPLICATE_SUPPRESSED:MULTILAYER_TRUE_DUPLICATE_OPEN")
                return false
            }
        }
        ...
        try {
            ...
            if (simErr != null) {
                if (simErr.startsWith("RPC error:") || simErr.startsWith("Simulate failed: null")) {
                    ...
                    try { PipelineHealthCollector.labelInc("RPC_SIMULATION_UNAVAILABLE") } catch (_: Throwable) {}
    ...
    internal fun doSell(ts: TokenState, reason: String,
        ...
        val configMode = if (isPaperRT()) "paper" else "live"
                ...
                sweep@ for (slip in slippageLevels) {
                    ...
                    try {
                        ...
                        sweepSig = wallet.signSendAndConfirm(
                            txResultSweep.txBase64, useJito, effectiveJitoTipLamports(c, urgent = true),
    ...
    fun sellOrphanedToken(mint: String, qty: Double, wallet: SolanaWallet): Boolean {
        ...
        return try {
            ...
            val pumpSig = if (com.lifecyclebot.engine.sell.MemeVenueRouter.isPumpRouteInvalid(mint)) {
                ...
                priorityFeeSol = 0.0001,
                useJito = c.jitoEnabled,
                jitoTipLamports = effectiveJitoTipLamports(c, urgent = true),
                sellTradeKey = orphanKey,
                traderTag = "ORPHAN",
