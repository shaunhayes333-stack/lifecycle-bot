...
object FinalDecisionGate {
    ...
    fun evaluate(
        ...
        if (blockReason == null) {
            try {
                // V5.0.6307 — REVERTED the V5.0.6304 paper-mode bypass.
                // Operator directive 2026-07: "just revert the lane unchoke.
                // isnt that the logical step". The V5.0.6304 unchoke opened
                // paper-mode admissions on toxic lanes (SHITCOIN|S0-10,
                // MOONSHOT|S26-40) and drove rolling 50-WR from ~70% to 14%
                // with a -81% paper drawdown. Restore the pre-6304 behaviour:
                // LaneAutoPauseGuard.statusFor() hard-blocks in BOTH paper
                // and live once a lane is flagged as toxic. The 6069 in-guard
                // isPaused() bypass is preserved for OTHER callers that want
                // paper-only sampling (LLM Lab shadow proof), but the FDG
