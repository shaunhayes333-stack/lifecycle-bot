...
object MemeTraderFullAuditSweeper {
    ...
    fun auditSourceTree(root: File): List<AuditItem> {
        ...
        return listOf(
            ...
            item("PASS_D_REGIME_VOLATILITY_4261", persistence.contains("REGIME_TRANSITION") && persistence.contains("VOLATILITY_REGIME") && regime.contains("exportState") && executor.contains("RegimeVolatilityExecutorBridge.sizeShape") && src("src/main/kotlin/com/lifecyclebot/engine/RegimeVolatilityExecutorBridge.kt").contains("VolatilityRegimeAI.analyze"), "Regime/volatility state must persist and feed consumers"),
            ...
            item("PASS_F_EXPRESS_PARITY_4261", express.contains("restoreRide") && express.contains("EXPRESS_DAILY_LOSS_RECOVERY_PROBE_4223") && src("src/main/kotlin/com/lifecyclebot/engine/PositionPersistence.kt").contains("EXPRESS_RESTORED_ACTIVE_RIDE_4223"), "ShitCoinExpress must have live/paper state, restore, and probe parity"),
            ...
            item("PASS_I_HOLD_TIME_OPTIMIZER_4261", hold.contains("saveToJson") && hold.contains("loadFromJson") && persistence.contains("HOLD_TIME_OPTIMIZER") && (bot + executor).contains("holdTime"), "Hold-time intelligence must persist and affect entry/exit learning"),
