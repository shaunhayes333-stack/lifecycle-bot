...
object MemeTraderFullAuditSweeper {
    ...
    fun auditSourceTree(root: File): List<AuditItem> {
        ...
        return listOf(
            ...
            item("PASS_E_SUPERBRAIN_CONSUMPTION_4261", superBrain.isNotBlank() && (bot + executor + fdg).contains("SuperBrainEnhancements"), "SuperBrainEnhancements must influence actual trade surfaces, not sit as theatre"),
            ...
            item("PASS_G_SHADOW_LEARNING_4261", shadow.contains("addVariant") && (bot + executor + fdg).contains("ShadowLearningEngine"), "ShadowLearningEngine variants must be registered/consumed"),
            item("PASS_H_EXIT_MANAGER_DEDUP_4261", sellOpt.contains("recordExitOutcome") && executor.contains("SellOptimizationAI.recordExitOutcome") && !executor.contains("ExitManager.execute"), "Exit authority must avoid duplicate manager firing and feed sell optimization"),
            ...
            item("PASS_J_METACOGNITION_CONSUMPTION_4261", meta.isNotBlank() && (bot + executor + fdg).contains("MetaCognitionAI"), "MetaCognition grade must be consumed by real decision/sizing surfaces"),
        )
    }
