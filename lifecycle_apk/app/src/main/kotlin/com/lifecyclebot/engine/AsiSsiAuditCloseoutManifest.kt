...
object AsiSsiAuditCloseoutManifest {
    private val implemented = listOf(
        ...
        "A34 source-family opportunity scorecard", "A35 runner exit shadow ledger", "A36 live wallet growth governor", "A37 closeout manifest self-report",
        "A38 persistence sentinel", "A39 hot-path provider-call sentinel", "A40 learning fanout mux sentinel", "A41 operator KPI closeout report"
    )
    private val pending = emptyList<String>()

    fun status(): String = "ASI_SSI_AUDIT_CLOSEOUT_4292 implemented=${implemented.size} pending=${pending.size} latest=${implemented.takeLast(5).joinToString(";")} next=${pending.joinToString(";").ifBlank { "none" }} report_only=true audit_closed=true"
