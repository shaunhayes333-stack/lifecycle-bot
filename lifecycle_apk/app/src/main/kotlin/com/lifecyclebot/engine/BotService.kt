...
class BotService : Service() {
    ...
    private suspend fun botLoop() {
        ...
        while (status.running) {
          try {
            ...
            // Process all tokens in parallel — each gets its own coroutine.
            // This reduces per-cycle latency from (N×50ms + pollSeconds) to just pollSeconds.
            // V4.1: Token processing extracted to processToken() to reduce botLoop complexity
            val tokenJobs = prioritizedWatchlist.map { mint ->
              scope.launch {
                ...
                processToken(mint, cfg, wallet, lastSuccessfulPollMs)
              } // end scope.launch
            } // end map
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    /**
     * Process a single token in the watchlist.
     * V4.1: Extracted from botLoop lambda to reduce function complexity.
     * This was causing Kotlin compiler StackOverflowError during coroutine transformation.
     */
