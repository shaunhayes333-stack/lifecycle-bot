...
class BotService : Service() {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // FULL TRADE SIMULATION - Tests entire pipeline
    // ══════════════════════════════════════════════════════════════════════════
    
    fun runFullTradeSimulation() {
        scope.launch {
            try {
                addLog("═══════════════════════════════════════════")
                addLog("🧪 FULL TRADE SIMULATION STARTING")
                addLog("═══════════════════════════════════════════")
                ErrorLogger.info("Simulation", "Starting full trade simulation")
                
                val cfg = ConfigStore.load(applicationContext)
                val walletSol = walletManager.state.value.solBalance
                val solPrice = WalletManager.lastKnownSolPrice.takeIf { it > 0 } ?: 140.0
                
                addLog("📊 STEP 1: WALLET STATUS")
                addLog("   Wallet SOL: ${walletSol}")
                addLog("   SOL Price: $${"%.2f".format(solPrice)}")
                addLog("   Paper Mode: ${cfg.paperMode}")
                addLog("   Auto Trade: ${cfg.autoTrade}")
                ErrorLogger.info("Simulation", "Wallet: $walletSol SOL, Price: $solPrice, Paper: ${cfg.paperMode}")
                
                // STEP 2: SEARCH - Find a token
                addLog("")
                addLog("🔍 STEP 2: SEARCH - Finding token via DexScreener API")
                
                val testToken = findTestToken()
                if (testToken == null) {
                    addLog("❌ SEARCH FAILED: Could not find any token")
                    ErrorLogger.error("Simulation", "Search failed - no token found")
                    return@launch
                }
                
                addLog("   ✅ Found: ${testToken.symbol}")
                addLog("   Mint: ${testToken.mint.take(20)}...")
                addLog("   Liquidity: $${"%.0f".format(testToken.liquidityUsd)}")
                addLog("   Market Cap: $${"%.0f".format(testToken.mcapUsd)}")
                ErrorLogger.info("Simulation", "Found token: ${testToken.symbol} liq=${testToken.liquidityUsd}")
                
                // STEP 3: IDENTIFY - Add to watchlist
                addLog("")
                addLog("📋 STEP 3: IDENTIFY - Adding to watchlist")
                
                val wl = cfg.watchlist.toMutableList()
                if (testToken.mint !in wl) {
                    wl.add(testToken.mint)
                    ConfigStore.save(applicationContext, cfg.copy(watchlist = wl))
                    addLog("   ✅ Added ${testToken.symbol} to watchlist")
                } else {
                    addLog("   ℹ️ ${testToken.symbol} already in watchlist")
                }
                
                // Create token state
                val ts = status.tokens.getOrPut(testToken.mint) {
                    com.lifecyclebot.data.TokenState(
                        mint = testToken.mint,
                ...
                // STEP 4: QUALIFY - Safety & signal checks
                addLog("")
                addLog("🛡️ STEP 4: QUALIFY - Running safety checks")
                
                val safetyResult = safetyChecker.check(testToken.mint, testToken.symbol)
                addLog("   Safety Score: ${safetyResult.score}")
                addLog("   Is Safe: ${safetyResult.passed}")
                addLog("   Reason: ${safetyResult.reason}")
                ErrorLogger.info("Simulation", "Safety: score=${safetyResult.score} passed=${safetyResult.passed}")
                
                // Strategy evaluation
                addLog("")
                addLog("📈 STEP 5: STRATEGY EVALUATION")
                
                // Set some candle data for evaluation
                ts.lastLiquidityUsd = testToken.liquidityUsd
                ts.lastFdv = testToken.mcapUsd
                
                val stratResult = strategy.evaluate(ts, null)
                addLog("   Phase: ${stratResult.phase}")
                addLog("   Signal: ${stratResult.signal}")
                addLog("   Entry Score: ${"%.1f".format(stratResult.entryScore)}")
                addLog("   Exit Score: ${"%.1f".format(stratResult.exitScore)}")
                ErrorLogger.info("Simulation", "Strategy: phase=${stratResult.phase} signal=${stratResult.signal} entry=${stratResult.entryScore}")
                
                // STEP 6: FUND - Position sizing
                addLog("")
                addLog("💰 STEP 6: FUND - Position sizing")
                
                val sizeResult = SmartSizer.calculatePositionSize(
                    walletSol = walletSol,
                    entryScore = 70.0,  // Simulate good entry
                    safetyScore = safetyResult.score,
                    liquidityUsd = testToken.liquidityUsd,
                    avgWinPct = 0.5,
                    streak = 0,
                    config = cfg
                )
                addLog("   Position Size: ${sizeResult.sizeSol} SOL")
                addLog("   As USD: $${"%.2f".format(sizeResult.sizeSol * solPrice)}")
                addLog("   Explanation: ${sizeResult.explanation}")
                ErrorLogger.info("Simulation", "Size: ${sizeResult.sizeSol} SOL (${sizeResult.explanation})")
                
                // STEP 7: PURCHASE - Execute buy (simulated)
                addLog("")
                addLog("🛒 STEP 7: PURCHASE - ${if (cfg.paperMode) "PAPER" else "LIVE"} trade")
                
                if (cfg.paperMode) {
                    // Simulate paper trade
                    val buyPrice = 0.00001  // Simulated price
                    val tokensBought = (sizeResult.sizeSol * solPrice) / buyPrice
                    
                    ts.position = com.lifecyclebot.data.Position(
                        isOpen = true,
                        entrySol = sizeResult.sizeSol,
                        entryPriceUsd = buyPrice,
                        entryTimestamp = System.currentTimeMillis(),
                        currentPriceUsd = buyPrice,
                        tokenBalance = tokensBought,
                        highestPriceUsd = buyPrice
                    )
                    status.openPositionCount++
                    status.totalExposureSol += sizeResult.sizeSol
                    
                    addLog("   ✅ PAPER BUY executed")
                    addLog("   Entry: ${sizeResult.sizeSol} SOL @ $$buyPrice")
                    addLog("   Tokens: ${"%.0f".format(tokensBought)}")
                    addLog("   Position now OPEN")
                    ErrorLogger.info("Simulation", "Paper buy: ${sizeResult.sizeSol} SOL")
                    
                    // Simulate price increase for sell test
                    delay(1000)
                    
                    // STEP 8: SELL - Exit position
                    addLog("")
                    addLog("💸 STEP 8: SELL - Simulating exit")
                    
                    val exitPrice = buyPrice * 1.20  // 20% profit
                    val exitValueSol = (tokensBought * exitPrice) / solPrice
                    val pnlSol = exitValueSol - sizeResult.sizeSol
                    val pnlPct = (pnlSol / sizeResult.sizeSol) * 100
                    
                    addLog("   Exit Price: $${"%.8f".format(exitPrice)}")
                    addLog("   Exit Value: ${"%.4f".format(exitValueSol)} SOL")
                    addLog("   P&L: ${"%.4f".format(pnlSol)} SOL (${"%.1f".format(pnlPct)}%)")
                    ...
                    addLog("   ✅ PAPER SELL executed")
                    addLog("   Position now CLOSED")
                    ErrorLogger.info("Simulation", "Paper sell: PnL=$pnlSol SOL ($pnlPct%)")
                    
                    // STEP 9: REFUND - Treasury/compounding
                    addLog("")
                    addLog("🏦 STEP 9: REFUND - Treasury processing")
                    
                    val profitUsd = pnlSol * solPrice
                    addLog("   Profit: $${"%.2f".format(profitUsd)}")
                    addLog("   Treasury would lock: ${"%.1f".format(TreasuryManager.MILESTONES.getOrNull(0)?.lockPct?.times(100) ?: 20)}%")
                    addLog("   Remaining compounds back to wallet")
                    
                } else {
                    addLog("   ⚠️ Live trading - simulation stopped here")
                    addLog("   Enable paper mode to test full cycle")
                }
                ...
                addLog("═══════════════════════════════════════════")
                ErrorLogger.info("Simulation", "Full trade simulation completed successfully")
                
            } catch (e: Exception) {
                addLog("❌ SIMULATION ERROR: ${e.message}")
                ErrorLogger.error("Simulation", "Simulation failed: ${e.message}", e)
            }
        }
    }
    
    private suspend fun findTestToken(): SolanaMarketScanner.ScannedToken? {
        return kotlinx.coroutines.withContext(Dispatchers.IO) {
            try {
                ...
                val pairs = org.json.JSONObject(body).optJSONArray("pairs") ?: return@withContext null
                ...
                for (i in 0 until minOf(pairs.length(), 50)) {
                    ...
                    if (mint.isBlank() || mint.startsWith("0x")) continue
                    ...
                    return@withContext SolanaMarketScanner.ScannedToken(
                        mint = mint,
                        symbol = symbol,
                        name = p.optJSONObject("baseToken")?.optString("name", "") ?: symbol,
                        source = SolanaMarketScanner.TokenSource.RAYDIUM_NEW_POOL,
                        liquidityUsd = liq,
                        volumeH1 = p.optJSONObject("volume")?.optDouble("h1", 0.0) ?: 0.0,
                        mcapUsd = p.optDouble("marketCap", 0.0),
                ...
                ErrorLogger.error("Simulation", "findTestToken error: ${e.message}", e)
                null
            }
        }
    }
