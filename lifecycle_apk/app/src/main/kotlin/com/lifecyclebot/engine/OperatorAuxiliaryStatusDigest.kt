...
object OperatorAuxiliaryStatusDigest {
    fun status(): String {
        ...
        return "OPERATOR_AUX_STATUS_DIGEST_4364 tokenRefresh=[$tokenRefresh] birdeyeBudget=[$birdeyeBudget] apiHealth=[$apiHealth] fees=[$fees] exits=[$exits] liveTuner=[$liveTuner] scannerBrain=[$scannerBrain] strategyVariants=[$strategyVariants] exploration=[$exploration] noTrade=[$noTrade] sellFailures=[$sellFailures] sellJobs=[$sellJobs] report_only=true no_execution_authority=true no_gate_change=true"
    }
}
