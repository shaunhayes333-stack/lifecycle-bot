...
class Aate6616CausalP0RepairTest {
    ...
    @Test fun pre_v3_and_executor_consume_the_same_canonical_resolver() {
        ...
        assertEquals("entry and fresh exit hydration share the same resolver", 2,
            Regex("resolveBestSourceEvidence6734").findAll(executor).count())
        assertTrue(executor.contains("freshEconomicExit6737(ts.mint, now)"))
