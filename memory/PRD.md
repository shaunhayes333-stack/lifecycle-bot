...
| **5.0.4551** | **MEGA TRIAGE (operator audit P0).** V5.0.4545 dump: 55 closes WR=7.5% PnL=-0.2420 SOL with -96% catastrophic closes hidden behind staleUnproven=15. Three fixes: **(1) STALE_RECOVERY_UNPROVEN REVIVE** — new `HostWalletTokenTracker.reviveStaleUnprovenBotBuys()` promotes stale-unproven positions with provable bot-buy lineage back to `OPEN_BALANCE_PROOF_PENDING` so active sell-management resumes; called from init() + every reconcile tick. **(2) LIVE_PROBABILITY catastrophic-lane hard-stop** — when laneSamples≥4 AND (EV≤-40% OR (WR=0%+n≥6) OR EV≤-60%), `finalMult = 0.0` (no new exposure). EXPRESS/MANIPULATED/BLUECHIP from V5.0.4545 dump would all hard-stop under this. **(3) UNIVERSAL THIN-LIQ RUG BACKSTOP** — V5.0.4187 backstop extended: age window 60s→10min, added `liqDrainRug` path (liq<$2.5K AND PnL≤-15% regardless of age). CI #4547 GREEN. |
