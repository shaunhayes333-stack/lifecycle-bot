...
| **5.0.4187** | **TRIAGE bundle (operator P0): lease orphan + rug buy + bleed cap.** V5.0.4186 dump showed bot bought BCLiquFq → lost -96.1% (= every prior gain) AND 25+ buys orphaned their leases (silent early-returns post `lease.acquire`). Operator: "all 3". **A) LEASE ORPHAN RELEASE** — 7 silent early-return paths now properly close the lease (`buyTerminalFail` for terminal decisions, `releaseNonTerminal` for transient defers like MUTEX_BUSY / WALLET_BALANCE_ZERO). Frees 30s slot for immediate retry. **B1) REAL_PRICE_LOCK on BUY** — after `SecurityGuard.validateQuote`, if router contains 'PUMP' and `priceImpactPct == 0.0` (PumpPortal doesn't surface impact, so SecurityGuard's 3% gate is bypassed), request a Jupiter sanity quote. ABORT if Jupiter impact > 8% OR outAmount diverges > 25%. Failure-soft on Jupiter unavailable. **B2) THIN-LIQ EARLY-RUG BACKSTOP** — for tokens with liq < $5K in their first 60s on the book, hard-exit at -10% PnL (was -25%). CI #4187b GREEN (4187 first attempt failed on `outAmount.toDoubleOrNull()` — outAmount is Long not String, fixed). |
