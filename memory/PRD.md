...
## V5.0.6811 — AUTHORITY CONSOLIDATION (2026-02)

Operator diagnosis: "One candidate/version is still reaching multiple
authority-bearing lane/FDG paths after canonical ownership has already
been selected." Fixed items #1/2/4/5/6/7 in one pass.

- **New `CanonicalFdgAuthorityRegistry6811`**: one seal per
  `(mode, mint, candidateVersion)`. `claim()` returns `Accepted` on
  first arrival, `Duplicate(existing, attemptedLane)` when a sibling
  lane tries the same key. Idempotent for same-lane re-entry. TTL
  90s. Explicit `invalidate()` for legitimate material policy change.
- **`FinalDecisionGate.evaluate` gated at entry AND exit**:
    • Entry: if a different canonical lane already owns the seal for
      this `(mode, mint, candidateVersion)`, return a
      `FDG_SHADOW_OBSERVATION_6811` non-executable verdict — sibling
      lanes see the decision surface but cannot enter authoritative
      FDG, cannot create/reuse executable intents, cannot trigger
      sizing.
    • Exit: on winning ALLOW, `claim()` seals the executable notional
      so downstream `ExecutableOpenGate` + `OrderSizeResolver6441`
      consume the exact sealed value. Race losers are also converted
      to shadow.
  Counters: `FDG_AUTH_DECISION_6811`, `FDG_SHADOW_OBSERVATION_6811`,
  `FDG_AUTH_DUPLICATE_SUPPRESSED_6811`.
- **Sizing finality**: V5.0.6810 `intentSupersedes6809` already
  preserves the sealed intent when semantics are unchanged; the FDG
  claim now carries `sealedNotional` for downstream ticket=intent
  assertion.
- **`POLICY_NEG_EV_BLOCK_6801` minimum sample**: hard veto now requires
  ≥ `MIN_EV_HARD_VETO_SAMPLE_6811 = 3` independent attributable EV
  contributors. Single/low-sample negative EV becomes
  `POLICY_NEG_EV_ADVISORY_6811` (BUY-like action preserved, telemetry
  recorded). Hard safety (rug/liquidity/scam/route) is unchanged.
- **Acceptance tests added**:
    • `fdg_authority_registry_first_claim_wins_and_sibling_is_suppressed`
    • `fdg_authority_registry_same_lane_reentry_is_idempotent`
    • `negative_ev_low_sample_becomes_advisory_not_block`
    • Updated `negative_ev_aate_veto_never_becomes_buy` for the new
      3-contributor requirement.
- **NOT touched**: `CanonicalPositionAuthority6441`, paper capital
  conservation, economic schema/idempotency, `FinalizedTradeBus`,
  canonical reconciler, paper atomic commit, terminal mutation latch,
  lot quantity boundary, sell oversell protection, mode separation,
  LIVE execution safety, forced inventory classification (item #8),
  replay isolation (#9), decimal-skew learning quarantine (#10) — the
  operator explicitly restricted scope and existing invariants remain
  at zero violations.
- **CI status:** `Build AATE APK` **succeeds** for V5.0.6811 (16m38s).
  `Runtime Smoke Test` still fails on the pre-existing
  `NO_COMPLETED_PASSING_CURRENT_WINDOW` — unrelated, unchanged.
